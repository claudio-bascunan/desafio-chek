...
export function sendMailRestore(destinatario: string, token: string) {
  const enlaceRestablecer = `http://3.136.197.252:4200/reset-password?token=${token}`;
  const contenido = `
    ¡Hola!<br><br>
...
export function sendMailVerification(destinatario: string, token: string) {
  const enlaceVerificacion = `http://3.136.197.252:4200/validate-register?token=${token}`;
  const contenido = `
    ¡Hola!<br><br>
